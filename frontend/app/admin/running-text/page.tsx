...
export default function RunningTextPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold">Running Text Management</h1><p className="text-sm text-muted-foreground">Full CRUD Control - Enterprise Grade - Audit Logged</p></div>
      
      <Card>
        <CardHeader><CardTitle className="text-base">Tambah Running Text</CardTitle></CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader><CardTitle>Daftar Running Text ({items.length})</CardTitle></CardHeader>
        <CardContent>
          {loading ? <div>Loading...</div> : (
            <div className="space-y-2 max-h-[500px] overflow-y-auto">
              ...
              {items.length===0 && <p className="text-center text-muted-foreground py-8">Belum ada data - Running Text kosong</p>}
            </div>
          )}
        </CardContent>
      </Card>
