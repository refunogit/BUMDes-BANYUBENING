import { prisma } from '../../config/database';
import { parsePagination, getSkipTake, paginateResult } from '../../utils/pagination';
import { createAuditLog } from '../../lib/audit';
import { AppError } from '../../middlewares/error.middleware';
import { queueService } from '../../config/redis';


export const runningTextService = {
  async list(query: any) {
    const pagination = parsePagination(query);
    const { skip, take } = getSkipTake(pagination);
    const where: any = {};
    if (query.search) {
      where.OR = [
        { text: { contains: query.search, mode: 'insensitive' } },
        { emoji: { contains: query.search, mode: 'insensitive' } },
        { isActive: { contains: query.search, mode: 'insensitive' } },
        { order: { contains: query.search, mode: 'insensitive' } },
        { speed: { contains: query.search, mode: 'insensitive' } },
      ];
    }
    if (query.isActive !== undefined) where.isActive = query.isActive === 'true';
    if (query.isPublished !== undefined) where.isPublished = query.isPublished === 'true';

    const [data, total] = await Promise.all([
      prisma.runningText.findMany({ where, skip, take, orderBy: { [pagination.sortBy]: pagination.sortOrder } as any as any }),
      prisma.runningText.count({ where }),
    ]);
    return paginateResult(data, total, pagination);
  },

  async getById(id: string) {
    const item = await prisma.runningText.findUnique({ where: { id } });
    if (!item) throw new AppError(404, 'RunningText not found');
    return item;
  },


  async create(data: any,  userId?: string, ip?: string, userAgent?: string) {
    const payload: any = { ...data };
    // Normalize booleans and numbers
    if (payload.isActive !== undefined) payload.isActive = payload.isActive === 'true' ? true : payload.isActive === 'false' ? false : payload.isActive;
    if (payload.isPublished !== undefined) payload.isPublished = payload.isPublished === 'true' ? true : payload.isPublished === 'false' ? false : payload.isPublished;
    if (payload.order !== undefined) payload.order = parseInt(payload.order, 10);
    if (payload.progress !== undefined) payload.progress = parseInt(payload.progress, 10);
    if (payload.speed !== undefined) payload.speed = parseInt(payload.speed, 10);
    if (payload.budget !== undefined) payload.budget = isNaN(parseFloat(payload.budget)) ? undefined : parseFloat(payload.budget);



    const created = await prisma.runningText.create({ data: payload });

    await createAuditLog({ userId, action: 'CREATE', entity: 'RunningText', entityId: created.id, newValue: created, ipAddress: ip, userAgent });
    await queueService.delCache('runningText:list');
    return created;
  },

  async update(id: string, data: any,  userId?: string, ip?: string, userAgent?: string) {
    const existing = await prisma.runningText.findUnique({ where: { id } });
    if (!existing) throw new AppError(404, 'RunningText not found');

    const payload: any = { ...data };
    if (payload.isActive !== undefined) payload.isActive = payload.isActive === 'true' ? true : payload.isActive === 'false' ? false : payload.isActive;
    if (payload.isPublished !== undefined) payload.isPublished = payload.isPublished === 'true' ? true : payload.isPublished === 'false' ? false : payload.isPublished;
    if (payload.order !== undefined) payload.order = parseInt(payload.order, 10);
    if (payload.progress !== undefined) payload.progress = parseInt(payload.progress, 10);
    if (payload.speed !== undefined) payload.speed = parseInt(payload.speed, 10);
    if (payload.budget !== undefined) payload.budget = isNaN(parseFloat(payload.budget)) ? undefined : parseFloat(payload.budget);



    const updated = await prisma.runningText.update({ where: { id }, data: payload });

    await createAuditLog({ userId, action: 'UPDATE', entity: 'RunningText', entityId: id, oldValue: existing, newValue: updated, ipAddress: ip, userAgent });
    await queueService.delCache('runningText:list');
    return updated;
  },

  async delete(id: string, userId?: string, ip?: string, userAgent?: string) {
    const existing = await prisma.runningText.findUnique({ where: { id } });
    if (!existing) throw new AppError(404, 'RunningText not found');
    await prisma.runningText.delete({ where: { id } });
    await createAuditLog({ userId, action: 'DELETE', entity: 'RunningText', entityId: id, oldValue: existing, ipAddress: ip, userAgent });
    await queueService.delCache('runningText:list');
    return { message: 'RunningText deleted' };
  },
};
