...
export const programKerjaService = {
  async list(query: any) {
    ...
    if (query.search) {
      where.OR = [
        ...
        { budget: { contains: query.search, mode: 'insensitive' } },
        { progress: { contains: query.search, mode: 'insensitive' } },
    ...
    const [data, total] = await Promise.all([
      prisma.programKerja.findMany({ where, skip, take, orderBy: { [pagination.sortBy]: pagination.sortOrder } as any as any }),
  ...
  async getById(id: string) {
    const item = await prisma.programKerja.findUnique({ where: { id } });
    if (!item) throw new AppError(404, 'ProgramKerja not found');
    return item;
  },

  async getBySlug(slug: string) {
    const item = await prisma.programKerja.findUnique({ where: { slug } });
    if (!item) throw new AppError(404, 'ProgramKerja not found');
    return item;
  },
  ...
  async create(data: any, imageUrl?: string, userId?: string, ip?: string, userAgent?: string) {
    ...
    const created = await prisma.programKerja.create({ data: payload });

    await createAuditLog({ userId, action: 'CREATE', entity: 'ProgramKerja', entityId: created.id, newValue: created, ipAddress: ip, userAgent });
    await queueService.delCache('programKerja:list');
    return created;
  },
  ...
  async update(id: string, data: any, imageUrl?: string, userId?: string, ip?: string, userAgent?: string) {
    ...
    if (!existing) throw new AppError(404, 'ProgramKerja not found');
    ...
    const updated = await prisma.programKerja.update({ where: { id }, data: payload });

    await createAuditLog({ userId, action: 'UPDATE', entity: 'ProgramKerja', entityId: id, oldValue: existing, newValue: updated, ipAddress: ip, userAgent });
    await queueService.delCache('programKerja:list');
    return updated;
  },
  ...
  async delete(id: string, userId?: string, ip?: string, userAgent?: string) {
    ...
    await createAuditLog({ userId, action: 'DELETE', entity: 'ProgramKerja', entityId: id, oldValue: existing, ipAddress: ip, userAgent });
    await queueService.delCache('programKerja:list');
    return { message: 'ProgramKerja deleted' };
  },
};
